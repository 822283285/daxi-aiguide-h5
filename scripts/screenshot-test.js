...
const URL = 'https://html.qkbyte.cn/daxi/';
...
async function testPage() {
  ...
  try {
    ...
    // 检查是否有全局错误
    const hasViteIsModernBrowser = await page.evaluate(() => typeof window.__vite_is_modern_browser !== 'undefined');
    ...
    console.log('  - #first_page:', firstPageElement ? (firstPageVisible ? '✅ 存在且可见（加载中）' : '✅ 存在但已隐藏（正常）') : '❌ 不存在（已隐藏）');
    ...
    console.log('  - #first_page (3 秒后):', firstPageVisibleAfter ? '✅ 可见（可能卡住）' : '✅ 已隐藏（正常）');
