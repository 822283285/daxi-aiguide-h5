...
export function createAppInitUsecase(options = {}) {
  ...
  return {
    start() {
      if (!appApi || typeof appApi.init !== "function") {
        return { initialized: false, reason: "appApi-unavailable" };
      }
      ...
      appApi.init({
        container: "app",
        pages: pages.map(createPageState),
        onCreate,
      });

      dispatchFirstScreenReady(bdid);
      return { initialized: true, buildingId: bdid };
    },
  };
}
