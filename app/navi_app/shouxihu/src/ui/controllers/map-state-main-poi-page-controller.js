...
export function createMapStateMainPoiPageController(options = {}) {
  const deps = resolveDeps(options);
  const { globalRef, domUtils, MapStateClass, SearchViewCtor, CommonPoiViewCtor, PoiTypeListViewCtor } = deps;
  ...
  return MapStateClass.extend({
    ...
    updateData: function (bdid) {
      ...
      let projDataPath = params.dataRootPath || `../projdata/${params.token}/{{bdid}}/{{filename}}`;
      let url = projDataPath;
...
export function registerMapStateMainPoiPageController(options = {}) {
  const deps = resolveDeps(options);
  const controller =
    options.controller ||
