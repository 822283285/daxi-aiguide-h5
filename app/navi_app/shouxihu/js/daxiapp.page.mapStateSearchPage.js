(function (global) {
   ...
   */
  const buildSearchCommand = (method, data) => ({
    retVal: "OK",
    method,
    data,
  });
  ...
  /** 获取缓存键名 */
  const getCacheKey = (token, bdid) => `${token}_${bdid}Search`;

  const MapStateSearchPage = MapStateClass.extend({
    __init__: function () {
      this._super();
    ...
    initialize: function (app, container) {
      ...
      const basicMap_html = '<div id="search_page" class="dx_full_frame_container"></div>';
      ...
      thisObject._searchView = new daxiapp.DXSearchComponent(app, thisObject._dom);
      ...
      const searchWraperhtml = '<div class="main_content" style="position: relative;flex-grow: 1;"><div class="search_display_wrapper"></div></div>';
      ...
      // 热搜组件
      thisObject._hotSearch = new daxiapp.DXHotSearchComponent(app, thisObject._disWrapper);
    ...
    _buildStartPoint: function () {
      ...
      if (!locationManager) return startPoint;
    ...
    _openAmapNavi: function (e) {
      ...
      window.AlipayJSBridge?.call("amapOpenPage", {
        ...
        urlParams,
        isDialog: false,
        complete: (res) => alert(JSON.stringify(res)),
        fail: (res) => alert(res.errorMessage),
      });
    },
    ...
    getHotData: function () {
      ...
      DXMapUtils.getDataBySecurityRequest(
        ...
        (res) => {
          ...
          if (!parsedData.list?.length) return;
          ...
          this._hotKeyWordWrap.html(`
            <div class="hotWrapper">
              <p class="title" style="line-height: 2;font-size: 1.4rem;">热门关键字</p>
    ...
    searchPoi: function (keyword) {
      const thisObject = this;
      const { token, bdid, floorId, defStartPoint } = thisObject.params;
      let { position, type } = thisObject.params;
      ...
      const _search = thisObject._app._mapView._search;
      _search?.cancel();
      ...
      if (hideSearchDis) params.hideDis = true;
      ...
      _search.query(
        ...
        (data) => {
          if (data && !lon && !lat) {
            data.forEach((item) => {
              if (item.dataType == "Gaode") item.dataType = 11;
    ...
    getHotSearchData: function (callbackFn) {
      daxiapp.api.getSearchHotWords({ bdid: this.bdid }, (data) => {
        if (data?.list?.length) callbackFn?.(data.list);
      });
    },

    getHistoryData: function () {
      const cacheKey = getCacheKey(this.token, this.bdid);
      const data = daxiapp.cache.history.getAll(cacheKey);
    ...
    addHistory: function (_data) {
      const cacheKey = getCacheKey(this.token, this.bdid);
      daxiapp.cache.history.add(_data, cacheKey);
    },

    clearHistory: function () {
      const cacheKey = getCacheKey(this.token, this.bdid);
      daxiapp.cache.history.clear(cacheKey);
    ...
    onStateBeginWithParam: function (args) {
      ...
      this._searchView.setPlaceholder(this.bdid ? "请输入要查询景区内位置" : "请输入要查询景区外位置");
  ...
  daxiapp.MapStateSearchPage = MapStateSearchPage;
})(window);
