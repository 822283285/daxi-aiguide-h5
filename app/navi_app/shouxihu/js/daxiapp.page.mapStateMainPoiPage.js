(function (global) {
  ...
  var MapStateMainPoiPage = MapStateClass.extend({
    __init__: function () {
      this._super();
    ...
    initialize: function (app, container) {
      ...
      thisObject._searchView = new daxiapp["DXSearchViewComponent2"](app, thisObject._wrapper);
      ...
      thisObject._commonPoiView = new daxiapp["DXCommonPOIViewComponent"](app, thisObject._mainContainerDom);
      thisObject._commonPoiView.init({
        onItemBtnClicked: function (sender, data) {
          var args = {
            method: "openSearchPage",
            retVal: "OK",
            data: data,
      ...
      thisObject._poiTypeListView = new daxiapp["DXPoiTypeListViewComponent"](app, thisObject._mainContainerDom);
    ...
    updateData: function (bdid) {
      ...
      if (thisObject.configData[key]) {
        ...
        if (bdid.length != 0) {
          ...
          url += "?t=" + (window["version"] || new Date().getTime());
        } else {
          url = url.replace("{{bdid}}", "appConfig").replace("{{filename}}", "app.json");
          url += "?t=" + (window["version"] || new Date().getTime());
        }
        thisObject._app.downloader.getServiceData(url, "get", "json", data, function (data) {
          thisObject.configData[key] = data;
          var mainPageData = data["mainPoiPage"];
          var commonPois = mainPageData["commonPois"];
          var poiTypeList = mainPageData["poiTypeList"];
          thisObject._commonPoiView.updateData(commonPois);
          thisObject._poiTypeListView.updateData(poiTypeList);
    ...
    openSearchPage: function () {
      ...
      app.mapStateManager.pushState("MapStateSearchPage", args);
    },
  });

  daxiapp["MapStateMainPoiPage"] = MapStateMainPoiPage;
})(window);
