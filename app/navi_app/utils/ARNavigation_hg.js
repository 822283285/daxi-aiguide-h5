(function(global){
    ...
    function create3dMan(){
        ...
        mtlLoader.load('../assets/models/333.mtl', function (materials) {
            ...
            objLoader.load('../assets/models/333.obj', function (object) {
                man3D = object;
                man3D.scale.set(0.015, 0.015, 0.015);
                man3D.rotation.x = Math.PI / 2;
            })
        })
    }
    ...
    function ARNavigation(parentDom) {
        ...
        function createArMap() {
            ...
            //灯光
            const hemiLight = new THREE.HemisphereLight( 0xcccccc, 0xffffff,1);
            ...
            if (coordinates.length !== 0) {
                ...
                for (let i = 1; i < coordinates.length; i++) {
                    ...
                    if (distance >= 1) {
                        ...
                        cont = cont+distance;
                        createLine(starPoint, distance, angle,flag);
                        flag = false;
                        starPoint.x = x
                        starPoint.y = y
                        if(cont>=15){
        ...
        function createLine(starPoint, length, angle,flag) {
            for (let i = 0.1; i <= length; i+=3) {
                ...
                if(i===0.1 && flag){
                    if(man3D){
                        man3D.position.set(x, y, 1)
                        if(!lastRotation){
                            lastRotation = angle - Math.PI / 2;
                            man3D.rotateY(lastRotation);
                        }else{
                            if(lastRotation !==  angle - Math.PI / 2){
                                man3D.rotateY(angle - Math.PI / 2 - lastRotation)
                                lastRotation = angle - Math.PI / 2;
                            }
                        }
                        if(!isShow) {
