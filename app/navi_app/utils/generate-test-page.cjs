/**
 * daxiapp.utils.js 浏览器测试页面
 * 用于在浏览器中测试 daxiapp.utils.js 的功能
 */
...
<head>
    ...
    <title>daxiapp.utils.js 功能测试</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>🧪 daxiapp.utils.js 功能测试</h1>
        
        <div class="info-box">
            <strong>ℹ️ 测试说明：</strong> 本页面测试 daxiapp.utils.js 导出的 API 功能，包括 DXUtils 工具库、Vector3 向量运算、Matrix 矩阵运算和 navi_utils 导航工具等。
        </div>
    ...
    <script type="module">
        ...
        function renderResults() {
            ...
            const sections = {
                'DXUtils 工具函数': testResults.slice(0, 14),
                'Vector3 向量运算': testResults.slice(14, 24),
                'Matrix 矩阵运算': testResults.slice(24, 28),
                'navi_utils 导航工具': testResults.slice(28, 34),
                '其他功能': testResults.slice(34)
            };
...
const outputPath = path.join(__dirname, 'test-daxiapp-utils.html');
fs.writeFileSync(outputPath, htmlContent, 'utf-8');

console.log('✅ 浏览器测试页面已生成:', outputPath);
console.log('📍 在浏览器中打开此文件即可运行测试');
