...
((global) => {
   ...
   */
  const DXIndoorMapScene = ((DXSceneNode) => {
    const DXIndoorMapScene = DXSceneNode.extend({
      /**
       * 初始化室内场景
       */
      __init__: function () {
       ...
       */
      _create: function (mapSDK, mapUrl, id, bdid, extent, bdInfo, mapStyle) {
        this.bdInfo = bdInfo;
        this.mapStyle = mapStyle || "default";
        this.styleVersion = bdInfo.styleVersion || global.version || Date.now();
        this.baseUrl = mapUrl;
        this.id = id;
        this.bdid = bdid;
        this.center = [];
       ...
       */
      _parseFloorInfo: function (context, floorObject) {
        const indoorMapApi = this._mapSDK._coreMap._indoorMapApi;
        ...
        if (wmtslayers) {
          wmtslayers.forEach((layerJson, j) => {
            const app = global.DaxiApp || global.daxiapp;
        ...
        if (vectorLayersJson) {
          vectorLayersJson.forEach((vectorLayerJson, j) => {
            ...
            if (global.version) {
              vectorDataUrl += (vectorDataUrl.indexOf("?") != -1 ? "&t=" : "?t=") + global.version;
            }
       ...
       * @param {Function} callback
       */
      _loadSceneConfigData: function (link, callback) {
       ...
       * @param {Function} callback
       */
      _setMapStyle: function (styleName, callback) {
       ...
       * @param {Boolean} force
       */
      _changeFloor: function (floorId, explodedView, force) {
        ...
        if (explodedView) {
          ...
          if (this.status == LOADED) {
            const visitor = explodedView ? global.DXChangeFloorWithExplodedViewVisitor?.(this, floorId) : global.DXChangeFloorVisitor?.(this, floorId);
            visitor?.visit();
          }
        }
       ...
       * @param {String} floorId
       */
      _updateFloorView: function (floorId) {
       ...
       * @param {String} floorId
       */
      _resetFloorView: function (floorId) {
       ...
       * @returns {Number}
       */
      _getFloorIndex: function (floorId) {
       ...
       */
      _toUpperFloor: function (floorOffset) {
       ...
       * @param {Number} floorOffset
       */
      _toLowerFloor: function (floorOffset) {
        const currentFloorInfo = this._getFloorInfo(this.currentFloorId);
        const currentFloorIndex = currentFloorInfo.flindex;
       ...
       * @param {Boolean} explodedView
       */
      _resumeFloor: function (explodedView) {
        if (explodedView) {
          this._updateFloorViewAnimated(this.currentFloorId);
        } else {
          ...
          if (this.status == LOADED) {
            const visitor = explodedView
              ? global.DXChangeFloorWithExplodedViewVisitor?.(this, this.currentFloorId)
              : global.DXChangeFloorVisitor?.(this, this.currentFloorId);
            visitor?.visit();
          }
        }
        this._mapSDK._coreMap.redraw();
       ...
       */
      _setFloorInterval: function (interval) {
       ...
       * @param {Number} offset
       */
      _setCurrentOffset: function (offset) {
       ...
       */
      _setFloorsOpacity: function (floorId, activeOpacity, inactiveOpacity) {
       ...
       */
      _setFloorsOffset: function (offset, currentFloorIndex, floorInterval) {
        this.floorInfos.forEach((floorInfo, i) => {
          const floorObject = this.getChildById(this.bdid + floorInfo.flid);
          if (floorObject) {
        ...
        global.DXUpdateHeightOffsetVisitor?.(this).visit();
      },
       ...
       * @param {String} floorId
       */
      _updateFloorViewAnimated: function (floorId) {
        const targetFloorInterval = this.floorInterval || this._mapSDK.config.floorInterval;
        const currentFloorIndex = this._getFloorIndex(this.currentFloorId);
        ...
        this._timer = setInterval(() => {
          if (offsetStep == 0 || (offsetStep > 0 && this.currentOffset > this.targetOffset) || (offsetStep < 0 && this.currentOffset < this.targetOffset)) {
            ...
            if (this.status == LOADED) {
              global.DXChangeFloorWithExplodedViewVisitor?.(this, floorId).visit();
            }
            return;
          }
          this._setFloorsOffset(this.currentOffset, currentFloorIndex, this.floorInterval);
       ...
       * @param {Boolean} explodedView
       */
      _updateFloors: function (explodedView) {
       ...
       */
      _visitor: function (type, floorId, visible) {
