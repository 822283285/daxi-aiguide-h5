/**
 * @fileoverview 路线相关 API
 * @description 提供展览路线数据查询功能
 * @author daxi
 * @created 2026-02-26
 */
 ...
 * @example
 * // 获取展览路线
 * const routes = await routeApi.getRouteAll({});
 */
export async function getRouteAll(options = {}) {
  ...
  if (!requestToken || !requestBdid) {
    console.warn('[RouteAPI] getRouteAll: token 或 bdid 为空');
  ...
  try {
    ...
    console.log('[RouteAPI] 获取展览路线成功', result);
    return result;
  } catch (error) {
    console.error('[RouteAPI] 获取展览路线失败:', error);
    throw error;
  }
}
